...
describe("TurkBar", function () {
  before(async function () {
    this.TurkToken = await ethers.getContractFactory("TurkToken")
    this.TurkBar = await ethers.getContractFactory("TurkBar")
  ...
  beforeEach(async function () {
    ...
    this.bar = await this.TurkBar.deploy(this.turk.address)
    this.turk.mint(this.alice.address, "100")
    this.turk.mint(this.bob.address, "100")
    this.turk.mint(this.carol.address, "100")
  })

  it("should not allow enter if not enough approve", async function () {
    await expect(this.bar.enter("100")).to.be.revertedWith("ERC20: transfer amount exceeds allowance")
    await this.turk.approve(this.bar.address, "50")
    await expect(this.bar.enter("100")).to.be.revertedWith("ERC20: transfer amount exceeds allowance")
    await this.turk.approve(this.bar.address, "100")
  ...
  it("should work with more than one participant", async function () {
    await this.turk.approve(this.bar.address, "100")
    await this.turk.connect(this.bob).approve(this.bar.address, "100", { from: this.bob.address })
    ...
    expect(await this.turk.balanceOf(this.bar.address)).to.equal("30")
    // TurkBar get 20 more SUSHIs from an external source.
    await this.turk.connect(this.carol).transfer(this.bar.address, "20", { from: this.carol.address })
    ...
    expect(await this.turk.balanceOf(this.bar.address)).to.equal("52")
    expect(await this.turk.balanceOf(this.alice.address)).to.equal("70")
    expect(await this.turk.balanceOf(this.bob.address)).to.equal("98")
  })
})
