...
describe("MasterChefV2", function () {
  before(async function () {
    await prepare(this, ['MasterChef', 'TurkToken', 'ERC20Mock', 'MasterChefV2', 'RewarderMock', 'RewarderBrokenMock'])
    await deploy(this, [
  ...
  beforeEach(async function () {
    ...
    await deploy(this,
      ...
      ['chef', this.MasterChef, [this.turk.address, this.bob.address, getBigNumber(100), "0", "0"]]
    ])
    ...
    await deploy(this, [
        ['chef2', this.MasterChefV2, [this.chef.address, this.turk.address, 1]],
  ...
  describe("PendingTurk", function() {
    it("PendingTurk should equal ExpectedTurk", async function () {
      await this.chef2.add(10, this.rlp.address, this.rewarder.address)
      ...
      await advanceBlock()
      let expectedTurk = getBigNumber(100).mul(log2.blockNumber + 1 - log.blockNumber).div(2)
      let pendingTurk = await this.chef2.pendingTurk(0, this.alice.address)
      expect(pendingTurk).to.be.equal(expectedTurk)
    })
    it("When block is lastRewardBlock", async function () {
      ...
      let log2 = await this.chef2.updatePool(0)
      let expectedTurk = getBigNumber(100).mul(log2.blockNumber - log.blockNumber).div(2)
      let pendingTurk = await this.chef2.pendingTurk(0, this.alice.address)
      expect(pendingTurk).to.be.equal(expectedTurk)
    })
  })
    ...
    it("Should emit event LogUpdatePool", async function () {
      ...
      await expect(this.chef2.updatePool(0))
            ...
            .withArgs(0, (await this.chef2.poolInfo(0)).lastRewardBlock,
              ...
              (await this.chef2.poolInfo(0)).accTurkPerShare)
    })
  ...
  describe("Harvest", function () {
    it("Should give back the correct amount of SUSHI and reward", async function () {
        ...
        let expectedTurk = getBigNumber(100).mul(log2.blockNumber - log.blockNumber).div(2)
        expect((await this.chef2.userInfo(0, this.alice.address)).rewardDebt).to.be.equal("-"+expectedTurk)
        await this.chef2.harvest(0, this.alice.address)
        expect(await this.turk.balanceOf(this.alice.address)).to.be.equal(await this.r.balanceOf(this.alice.address)).to.be.equal(expectedTurk)
    })
    ...
    it("Harvest for SUSHI-only pool", async function () {
      ...
      expect((await this.chef2.userInfo(0, this.alice.address)).rewardDebt).to.be.equal("-"+expectedTurk)
      await this.chef2.harvest(0, this.alice.address)
      expect(await this.turk.balanceOf(this.alice.address)).to.be.equal(expectedTurk)
    })
  })
