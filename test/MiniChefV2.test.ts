...
describe("MiniChefV2", function () {
  before(async function () {
    await prepare(this, ['MiniChefV2', 'TurkToken', 'ERC20Mock', 'RewarderMock', 'RewarderBrokenMock'])
    await deploy(this, [
  ...
  beforeEach(async function () {
    ...
    await deploy(this,
      ...
      ['chef', this.MiniChefV2, [this.turk.address]],
    ...
    await this.turk.mint(this.chef.address, getBigNumber(10000))
    await this.lp.approve(this.chef.address, getBigNumber(10))
    await this.chef.setTurkPerSecond("10000000000000000")
  ...
  describe("PendingTurk", function() {
    it("PendingTurk should equal ExpectedTurk", async function () {
      ...
      let expectedTurk = BigNumber.from("10000000000000000").mul(timestamp2 - timestamp)
      let pendingTurk = await this.chef.pendingTurk(0, this.alice.address)
    ...
    it("Should emit event LogUpdatePool", async function () {
      ...
      await expect(this.chef.updatePool(0))
            ...
            .withArgs(0, (await this.chef.poolInfo(0)).lastRewardTime,
              ...
              (await this.chef.poolInfo(0)).accTurkPerShare)
    })
  ...
  describe("Harvest", function () {
    it("Should give back the correct amount of SUSHI and reward", async function () {
        ...
        let timestamp = (await ethers.provider.getBlock(log.blockNumber)).timestamp
        let expectedTurk = BigNumber.from("10000000000000000").mul(timestamp2 - timestamp)
        expect((await this.chef.userInfo(0, this.alice.address)).rewardDebt).to.be.equal("-"+expectedTurk)
