...
// TurkMakerKashi is MasterChef's left hand and kinda a wizard. He can cook up Turk from pretty much anything!
// This contract handles "serving up" rewards for xTurk holders by trading tokens collected from Kashi fees for Turk.
contract TurkMakerKashi is Ownable {
    ...
    function _convertStep(address token0, uint256 amount0) private returns (uint256 turkOut) {
        if (token0 == turk) {
            IERC20(token0).safeTransfer(bar, amount0);
            turkOut = amount0;
        } else if (token0 == weth) {
            turkOut = _swap(token0, turk, amount0, bar);
        } else {
            ...
            turkOut = _convertStep(bridge, amountOut);
        }
    }
